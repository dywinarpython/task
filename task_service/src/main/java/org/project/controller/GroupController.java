...
public class GroupController {
    ...
    @GetMapping
    public Mono<ResponseEntity<ListGroupDto>> getGroups(@AuthenticationPrincipal Jwt jwt, @RequestParam Long page){
        return groupService.getGroups(jwt, page).map(ResponseEntity::ok);
    }
    ...
    @GetMapping("/{groupId}")
    public Mono<ResponseEntity<ListUserDto>> getUserFroGroup(@PathVariable Long groupId, @RequestParam Long page,  @AuthenticationPrincipal Jwt jwt){
        return groupUserService.getAllUserForGroup(groupId, jwt, page).map(ResponseEntity::ok);
    }
}
